...
3. **Dashboard — "Whereabouts"** (built — repo `activity-dash`, static Angular 22
   ...
   - Almanac back-matter (folios VII–X): **The Analyst** (nightly counsel
     ...
     Health shortcut → `/metrics`), **The Field Pen** (write events from
     the page: focus_start/focus_stop toggle + any snake_case event, source
     `field_pen`), and **The Call Sheet** (folio X — the declared weekly
     schedule, version-controlled in the dash repo's `lib/call-sheet.ts`,
     revised as the analyst's evidence accumulates).
