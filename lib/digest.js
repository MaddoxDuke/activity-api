...
function buildDigest(rows, { date, tz, nowMs, forceWeekly = false }) {
  ...
  for (const s of pool) {
    ...
    if (h <= 0) continue;
    const at = clock12InTz(Math.max(s.startMs, dayStart), tz);
    if (s.end === 'unknown') {
      dayLines.push(`${at} ${label(s.place)} — end unrecorded (gap)`);
    } else if (s.end === 'ongoing') {
      dayLines.push(`${at} ${label(s.place)} — still going`);
    } else {
      dayLines.push(`${at} ${label(s.place)} — ${fmtH(h)}`);
    }
  }
  ...
  const footer = [totals, streakLine, note].filter(Boolean);
...
function buildNote({ totalsBy, dayEventCount, date, tz, sessions }) {
  ...
  if (productive < 0.5) {
    line = pick([
      ...
      'A blank page — not the good kind. The bar, the bench and the shop all report you missing.',
